...
| 参数 | 现状 | 风险 |
|:---|:---|:---|
| `extrinsic_T` | [0, 0, 0.28] 默认猜测值 | **必须实车量测** G354→VLP-16 相对位置（注意是 IMU 系到 LiDAR 系，不是 base_link 到雷达的 0.56m） |
| `extrinsic_R` | 单位阵 | **可能不对（待核实）**：G354 为 z 朝下定义（mount_axes=y_front_x_left_z_down），VLP-16 正装 z 朝上——姿态不一致时外参旋转非单位阵，需按安装方向推导或实测 |
...
- FAST-LIO frame_id **写死** `camera_init`（odom 系）/`body`（IMU 系），无 base_frame 参数（KISS-ICP 有）
- **验证阶段不接 R2 TF 树**：FAST-LIO 独立链路，rviz 固定系选 `camera_init`
- 若后续集成：需静态 TF 桥 `camera_init ↔ odom`（设计任务，决策后单独做）
...
- [ ] **extrinsic_T / extrinsic_R 实车量测填值**（§三.1）
