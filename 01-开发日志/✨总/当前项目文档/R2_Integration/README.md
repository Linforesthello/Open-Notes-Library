...
| R2 底盘控制 | 串口键盘遥控，运动学正确 | 未接入 ROS2，未用 CAN |
...
```
当前：control/R2.py — 串口键盘遥控，独立运行
         PC 键盘 → USB 串口 → MCU → 电机
         运动学正确，但不在 ROS2 体系内

目标：ros2 topic pub /cmd_vel → R2 运动
         ROS2 /cmd_vel → chassis_can_node → CAN → MCLM → 电机
         里程计正解 → /odom_wheels
```
